#!/usr/bin/env python3
"""Push wiki pages to the GitHub Wiki repo."""
import pathlib
import subprocess
import os
...
def run(cmd, **kwargs):
    result = subprocess.run(cmd, shell=True, check=True, text=True, capture_output=True, **kwargs)
    if result.stdout:
...
def main():
    ...
    run(f'git clone "{wiki_url}" wiki')

    for filename, content in PAGES.items():
        pathlib.Path(f"wiki/{filename}").write_text(content)
        print(f"Written: {filename}")

    run("git add .", cwd="wiki")
    run(
        'git commit -m "wiki: add all pages (Home, Installation, Configuration, Metrics Reference, Grafana Dashboard, Troubleshooting, Release Process)"',
        cwd="wiki",
    )
    run("git push origin master", cwd="wiki")
    print("Wiki pages pushed successfully.")
